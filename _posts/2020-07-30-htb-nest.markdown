...
The smbclient command showed that there were several network shares. I went through and connected to each, and found that I was able to login and find possible Usernames in the Users share. I took note of this, as this information is sure to come up later for this box:
...
I did attempt to access the user shares, but access was denied for each of the directories. Continuing my enumeration of the network shares, I connected to Data, and found I could login to the Shared directory:
...
In this directory, I found a file, `Maintenance Alerts.txt`. I used the get command to downlaod the file to my local box, and viewed the file:
...
I have the RU_config.xml file, which contains the hash string that looked like base 64! I placed that file into the directory, and when I ran it, the program ran without any exception errors. I then placed a single line of code to write to console the Plain Text in the Utils.vb decrypt function:
...
And now, I am able to connect to the Users share and own user:
...
And now, with this password, I can telnet into the box and enable Debug:
...
Using the RU_config.xml and attempting to decrypt returned a padding error. Looking further into the KqdLdap.exe, I foudn the following code in the CR function:
...
I connected to SMB Share, but there was a shortcut link to the admins desktop. So, I used metasploit (after multiple failed attempts with evil-winrm, impacket psexec.py, and winexec) psexec with reverse_tcp payload:
